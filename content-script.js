        ...
        if(selectionText.length > 0){
            try{
              ...
              // // Define the base URL based on the mode
               const baseUrl = 'https://google-extension.onrender.com/api/check';
              ...
              // Make the fetch request
                ...
                let grammarBox = pattern.test(resultJson.output.correctedGrammar)? "No grammar error": dmp.diff_prettyHtml(diffs); 
        ...
        async function fetchParaphrase() {
          try {
           // const baseUrl = 'http://localhost:3001/api/para';
            const baseUrl = 'https://google-extension.onrender.com/api/para';
            // Make the fetch request
...
async function fetchRelatedWork() {
  try {
    ...
    const baseUrl = 'https://google-extension.onrender.com/api/abstract1';
    // Make the fetch request
    const result = await fetch(baseUrl);
    const resultJson = await result.json();
...
// check abstract 1
        ...
        async function fetchAbstract1() {
          ...
          async function fetchAbstract2() {
            ...
            const baseUrl = 'https://google-extension.onrender.com/check-paper/api/abstract2';
              // Make the fetch request
              const result = await fetch(baseUrl);
              const resultJson = await result.json();
